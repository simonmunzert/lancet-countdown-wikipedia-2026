  ...
  for (i in seq_along(pages)) {
    ...
    if (!file.exists(paste0(folder, filename))) {
      ...
      if(package == "pageviews"|class(pageviews_list[[i]])=="try-error"){
        pageviews_list[[i]] <- try(article_pageviews(project = paste0(language, ".wikipedia"), article = pages[i], start = str_replace_all(paste0(from, "00"), "-", ""), end = str_replace_all(paste0(to, "00"), "-", ""), reformat = TRUE))
      }
      if(!(package %in% c("wikipediatrend", "pageviews"))){
        stop("Argument 'package' incorrectly specified.")
      }
      if (!inherits(pageviews_list[[i]], "try-error")) {
        write.csv(pageviews_list[[i]], file = paste0(folder, filename), row.names = FALSE)
      }
  }
  }
